...
локального Tombstone (ADR-0005), продовжена на колективний канал: уже
опубліковане прибирається саме так, і жоден наступний синк його не воскрешає.
_Avoid_: тихе повторне імпортування, видалення спостережених рядків

**Черга модерації (Candidate)**:
Черга `pending_submissions`, у яку застосунок кладе **кандидата** —
перевірене реальним програванням надходження, яке ще не стало карткою
спільного каталогу. Ключ — `sha256(canonicalUrl)`, автор — **хеш** uid;
рішення («схвалено» / «відхилено») ухвалює куратор через бота, і клієнт
не має права ні змінювати стан, ні переписувати документ. Відхилене
потрапляє в блокліст і не повертається (ADR-0050, спека 2026-09-15).
_Avoid_: «уже в спільній базі» про кандидата, обіцянки публікації, сирий uid
...
**Replacement Mapping**:
Відома аудіозаглушка `reasd.org/notice/4read-notice.mp3` не є Source:
спільний HTTP-транспорт блокує її прямий URL і редиректи до неї до запиту
заглушки. Плеєр один раз шукає сумісне інше дозволене джерело, спершу серед
Sources того самого Edition. Заміна зберігає Chapter і позицію; відмовлене
джерело, інша начитка чи сама заглушка не можуть бути заміною.

Мапування Твору, чиє аудіо відмовлене чи відсутнє, на джерела інших джерел —
лише при торканні книжки: спершу нуль запитів по локальній union, спільному
SearchCache і локальному індексі творів, за промахом один паралельний
пошуковий залп усіма прямими джерелами, зведений за MergeKey і мемоїзований
6h/15m як вердикт Твору. Індекс може нести і session-backed BROWSER-джерело:
такий матч придатний лише за живої першосторонньої сесії (вона — робочий
транспорт, не нові браузерні двері); без неї запис пропускається. Узагальнення
Cross-resolve (#469): той самий шов і дисципліна, всі прямі джерела замість
одного. Медіатека мапується поступово сама; фонового обходу й пакетного
мапування не існує. Чесна невдача — «аудіо недоступне» плюс Source Watch, а не
браузерні двері.
_Avoid_: фоновий обхід, пакетне мапування, пошук на кожне джерело окремо, вигадане аудіо, нові браузерні двері з мапування
