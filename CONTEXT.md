...
The indexed Room read model used by «Огляд» filters. Work facets carry canonical author/series references and indexed genre memberships; Edition facets separately carry narrator, language, duration bucket, chapter count, full/abridged status and an expiring availability assertion. Edition availability is atomic (available, observed-at epoch milliseconds and bounded TTL), is stale at exact expiry, and never moves onto Work. Raw genre assertions keep their Source text and observation provenance beside the derived relation. `LocalFacetWriter` is the one bounded transactional write door, and `WorkFacetFilter` is the frozen Paging query contract: values inside one dimension are OR, dimensions are AND, and an empty dimension is unfiltered. It is a projection of Metadata Assertions, never a replacement Work, Edition or Source.
