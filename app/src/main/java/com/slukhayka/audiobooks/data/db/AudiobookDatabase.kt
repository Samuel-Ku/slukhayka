...
abstract class AudiobookDatabase : RoomDatabase() {
    ...
    companion object {
        ...
        /** Add notification state without replacing the shipped v25/v26 migrations. */
        internal val MIGRATION_26_27 = object : Migration(26, 27) {
            override fun migrate(db: SupportSQLiteDatabase) {
                val hasCount = db.query("PRAGMA table_info(person_bookmarks)").use { cursor ->
                    var found = false
                    while (cursor.moveToNext()) {
                        if (cursor.getString(cursor.getColumnIndexOrThrow("name")) == "lastNotifiedCount") found = true
                    }
                    found
                }
                if (!hasCount) db.execSQL("ALTER TABLE person_bookmarks ADD COLUMN lastNotifiedCount INTEGER NOT NULL DEFAULT 0")
        ...
        /**
         * Spec #462 Implementation Decision 6 (#467): v25 -> v26 adds the
         * `feed_snapshots` table — the persisted feed snapshot (source id,
         * feed key, page/cursor, fetched-at, JSON cards) the Огляд feeds read
         * before any network call. Pure addition: one new table, no existing
         * row is touched, so the migration is trivially safe on any v25
         * database. Internal (not private) so the JVM test suite can verify
         * the upgrade path against a real v25 database.
         */
            ...
            override fun migrate(db: SupportSQLiteDatabase) {
                db.execSQL(
                    "CREATE TABLE IF NOT EXISTS feed_snapshots (" +
                        ...
                        "pageCursor TEXT NOT NULL, " +
                        "fetchedAt INTEGER NOT NULL, " +
                        "cardsJson TEXT NOT NULL, " +
                        "PRIMARY KEY(sourceId, feedKey, pageCursor))"
                )
            }
        }

        /** Both historical v27 lanes converge without losing their own data. */
        internal val MIGRATION_27_28 = object : Migration(27, 28) {
            override fun migrate(db: SupportSQLiteDatabase) {
                MIGRATION_26_27.migrate(db)
                db.execSQL(
                    "CREATE TABLE IF NOT EXISTS popularity_assertions (" +
                        "id TEXT NOT NULL, " +
                        "kind TEXT NOT NULL, " +
                        "mergeKey TEXT NOT NULL, " +
...
/**
          * #823 — v47 -> v48 (перенумеровано: 45 зайнята readthroughs): the local search index. An FTS4 projection of
