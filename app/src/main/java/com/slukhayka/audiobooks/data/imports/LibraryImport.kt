...
class LibraryImport(
     ...
     * 2. Work identity: the captured title/author key must match the stored
     *    Work key, including equivalent HTML title encoding anchored to that
     *    existing key. No identity row is rewritten.
    ...
    suspend fun recoverWebSourcePage(
        ...
        if (!RecoveryIdentityGuard.matchesStoredWorkKey(
                storedTitle = existingBook.title,
                storedAuthor = existingBook.author,
                storedMergeKey = existingBook.mergeKey.orEmpty(),
                captured = detail
            )) return@withContext null
