...
class DurationEnrichmentRepositoryTest {
    ...
    private fun repo(adapter: FakeBookAdapter) =
        AudiobookRepository(dao, context, autoSyncOnInit = false, sourceAdapters = listOf(adapter))

    private fun runEnrich(repo: AudiobookRepository, batchLimit: Int = 5, now: Long = 1_700_000_000_000L): Int =
    ...
    fun `the pass throttles itself between runs`() {
        ...
        val repo = repo(adapter)
        val interval = AudiobookRepository.MIN_ENRICHMENT_INTERVAL_MS
