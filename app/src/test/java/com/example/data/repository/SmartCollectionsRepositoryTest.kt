...
/**
 * Spec-16 T2 (#108) — the module seam: smart collections recompute from
 * stored catalog state on the recompute trigger. Uses the fake DAO seam
...
class SmartCollectionsRepositoryTest {
    ...
    private fun module(dao: FakeAudiobookDao) = SmartCollections(
        dao = dao,
        loadSpecs = { fixture }
    )
    ...
    fun `matching books surface in their collections after a catalog sync`() = runTest {
        ...
        val collectionsModule = module(dao)

        collectionsModule.recompute()

        val collections = collectionsModule.matched.value
    ...
    fun `a catalog change recomputes the rows`() = runTest {
        ...
        collectionsModule.recompute()
        assertTrue(collectionsModule.matched.value.isEmpty())
    ...
    fun `an author-only entry stays absent while its author is ambiguous`() = runTest {
        ...
        val shevchenko = collectionsModule.matched.value.single { it.id == "shevchenko" }
    ...
    fun `nothing is persisted for the collection rows`() = runTest {
        ...
        assertEquals(1, collectionsModule.matched.value.size)
    }
}
