 ...
 * `PaneTitle`); the fixture uses Latin-only data, so a failure can only come
 * from chrome the app itself produced.
 */
...
class SeriesScreenEnglishChromeTest {
    ...
    private fun assertChromeHasNoCyrillic() {
        EnglishChromeWalk.assertNoCyrillic(composeTestRule, "series")
    }
}
