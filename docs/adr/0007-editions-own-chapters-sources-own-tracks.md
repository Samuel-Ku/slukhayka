...
## Уточнення 2026-09-14 — службове аудіо Lihtar

На сторінках Lihtar першим стоїть звук навігації `audio/name/click.mp3`,
а не запис книги. Адаптер бере лише `audio#player` з `audio/library/`.
Для збірок він читає прямі дочірні сторінки розділів у порядку меню.
Пропущений розділ робить весь результат неповним: наступні індекси не зсуваються.

Старий імпорт із єдиним службовим треком можна виправити під час відкриття
відтворення. Це вузький виняток: одне джерело Lihtar, один розділ і один
службовий трек. Заміна відбувається однією транзакцією з повторною перевіркою
старих рядків. Ідентичності книги й Edition, закладки, улюблене та швидкість
залишаються. Час прослуховування кліку й позначка завершення скидаються.
Інші топології автоматично не переписуються. Службовий трек не повертається
плеєру чи механізму заміщення навіть за невдалого відновлення.
