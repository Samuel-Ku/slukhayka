...
| S04 | Tap the card → play | Audio plays from `*.redirectto.cc` **with `Referer: https://sluhay.com/`** — 206 `audio/mpeg`, position advances, no 403; chapter auto-advance works | `spec14_t6_sluhay_04_playing.png`; logcat evidence: `WebSource` «Audio request in session: …track-N.mp3» lines AND `AudioPlayer` `HTTP 206 … Referer=https://sluhay.com/` lines (TransferListener, added for this checkpoint); a 403 here = the Referer seam regressed |
