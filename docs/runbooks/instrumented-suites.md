...
пристрої або емуляторі. У CI біжить **п'ять сюїт цього набору** — та сама
п'ятірка, що й локально, по одному gradle-виклику на клас. Решта класів
компілюється кожним прогоном і не виконується ніде. Це борг **#984**,
зафіксований списком у `InstrumentedCoverageGuardTest`: додати клас у прогін
можна лише явним рішенням, бо це ціна CI, а не код. Розширення з одного класу
до п'яти — #1017 (25.09), після того як пристрійна перевірка #852 зробила всі
п'ять зеленими.
...
І CI, і скрипт женуть по класу — і тепер це не лише зручність. Кожен
gradle-виклик `:app:connectedDebugAndroidTest` — це окремий інструментований
прогін, а отже окрема база: `IsolatedDatabaseTestRunner` чистить scratch-файл
в `onCreate`. Один спільний прогін на всі класи змішав би фікстури —
`MainActivityAccessibilityTest` навмисно перезаписує базу, і сусідній клас
побачив би не свої дані.

Історична причина — «кілька класів перебивають `UiAutomation` одне одному» —
не підтвердилась (#957 показав, що зайнятий `UiAutomation` давав зовнішній
демон, а не сусідні класи), але послідовний запуск лишається: він дає
пофайловий вердикт і не змішує логи. Фільтр
`-Pandroid.testInstrumentationRunnerArguments.class=` приймає й `package=`, якщо
треба прогнати пакет цілком.

## Межа CI

П'ятірка вище біжить у ніжці `Accessibility journey (API 35)` — її жене
`.github/scripts/run-accessibility-test.sh` (той самий список класів, по
одному gradle-виклику на клас, `--no-daemon --max-workers=1`).

Щоб у CI почав бігати **новий** клас, треба **дві** правки в одному PR:

1. додати його в масив `suites` у `.github/scripts/run-accessibility-test.sh`;
2. прибрати його з `KNOWN_UNEXECUTED_BY_CI` і дописати в `EXECUTED_BY_CI`
   у `InstrumentedCoverageGuardTest`.
