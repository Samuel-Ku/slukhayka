...
> Автономне інтерв’ю завершено 2026-09-05. Для нової реалізації
> канонічним є [цілісний дизайн](../../specs/2026-09-05-personal-recommendations-design.md):
> він визначає сигнали, початкові вподобання, стабільну добірку,
> колективний граф, згоду, дані, видалення, вимірювання й запуск.
> Нижче збережено історичні рішення від 2026-08-24; старі пороги згоди,
> поля й строки п’ятивагового контракту не слід переносити в новий API.
>
> Частково замінено 2026-09-05:
> [ADR-0030](../../adr/0030-opt-in-server-recommendation-profiles.md)
> дозволяє добровільний серверний профіль із творами, завершенням та
> оцінками, з можливістю видалення. Нижче збережено попередній дизайн;
> його заборона серверної історії й обмеження спільного навчання п’ятьма
> вагами більше не визначають новий напрям. Технічний контракт нового
> профілю ще проєктується; передавання даних залишається вимкненим.
>
> У [поточному інтерв’ю](../../specs/2026-09-05-personal-recommendations-discovery.md)
> також погоджено: явна оцінка твору переважає завершення та прогрес.
> Старе додавання їхніх ваг нижче потребує перегляду за цим правилом.
> Для першого запуску погоджено необов’язковий вибір 3–5 книг, які
> подобаються слухачеві, або кількох улюблених жанрів у тому самому кроці;
> після пропуску залишаються загальні добірки. Жанри впливають на порядок
> рекомендацій і не приховують решту каталогу.
> Ще не розпочаті книги з медіатеки тепер дозволені серед рекомендацій,
> зі зрозумілим статусом на картці; загальне виключення всіх Library Entry
> нижче для нового дизайну більше не діє.
