...
RULES_GATE=as-is RULES_MODE=run OUT_JSON=results-as-is.json \
  npx -y firebase-tools@14 emulators:exec --only firestore,auth --project spec40-matrix -- ./run-matrix.sh || true

# B) гейт відкритий ЛИШЕ в локальній копії для емулятора
cp ../../firestore.rules firestore.rules
RULES_GATE=open RULES_MODE=run OUT_JSON=results-open.json \
  npx -y firebase-tools@14 emulators:exec --only firestore,auth --project spec40-matrix -- ./run-matrix.sh || true
